...
import { formatCost, getAreaEffectTooltip } from '@shared/services/formatters';
import { getUsedCapacity, getWarheads } from '../services/ordnanceService';
...
export function MagazineOrdnanceEditForm({
  ...
  const handleReduceQuantity = (designId: string, qty: number) => {
    const existing = currentLoadout.find(item => item.designId === designId);
    if (!existing) return;
    if (existing.quantity <= qty) {
      handleUnload(designId);
    } else {
      updateLoadout(
