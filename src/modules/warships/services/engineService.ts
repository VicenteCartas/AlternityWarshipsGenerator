...
import { getEnginesData, getFuelTankData, getEngineAllowPowerGeneration } from '@shared/services/dataLoader';
import { generateId, interpolateByPercentage, validateMinSize, validateFuelTank } from '@shared/services/utilities';
 ...
 */
export function isEnginePowerGenerationAllowed(): boolean {
  return getEngineAllowPowerGeneration();
}

/**
 * Calculate power generated by an engine installation (house rule)
 * Returns 0 if the house rule is disabled or the engine has no valid powerGeneratedPerHullPoint
 */
export function calculateEnginePowerGenerated(engine: EngineType, hullPoints: number): number {
  if (!isEnginePowerGenerationAllowed()) return 0;
  if (typeof engine.powerGeneratedPerHullPoint !== 'number' || engine.powerGeneratedPerHullPoint <= 0) return 0;
  return Math.ceil(engine.powerGeneratedPerHullPoint * hullPoints);
}
...
export function calculateTotalEngineStats(
  ...
  for (const installation of installations) {
    ...
    totalPowerGenerated += calculateEnginePowerGenerated(installation.type, installation.hullPoints);
