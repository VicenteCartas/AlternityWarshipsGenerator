...
export const EDITOR_SECTIONS: EditorSection[] = [
  ...
  // ---- Support: Artificial Gravity Rules ----
  {
    id: 'artificialGravityRules',
    label: 'Artificial Gravity Rules',
    description: 'Rules that define which tech track + progress level combinations grant free artificial gravity as part of life support.',
    fileName: 'supportSystems.json',
    rootKey: 'artificialGravityRules',
    columns: [
      COL_ID, COL_NAME,
      { key: 'minProgressLevel', label: 'Min PL', type: 'number', required: true, min: 6, max: 9, width: 70, description: 'Minimum Progress Level required for this rule to apply' },
      COL_TECH, COL_DESC,
    ],
    defaultItem: { id: '', name: '', minProgressLevel: 7, techTracks: [], description: '' },
  },
  ...
  // ---- Defenses ----
  {
    ...
    columns: [
      ...
      { key: 'allowExtraUnits', label: 'Extra Units', type: 'boolean', width: 90, description: 'Allow extra units beyond full coverage for shield toughness upgrade' },
      { key: 'effect', label: 'Effect', type: 'text', width: 250, description: 'Special effect description' },
      COL_DESC,
    ],
    defaultItem: { id: '', name: '', progressLevel: 6, techTracks: [], category: 'screen', hullPoints: 1, hullPercentage: 0, powerRequired: 1, powerPer: 'unit', cost: 50000, costPer: 'unit', coverage: 0, effect: '', damageCheckBonus: 0, shieldPoints: 0, coverageMultiples: false, fixedCoverage: false, allowExtraUnits: false, description: '' },
  },
  ...
  // ---- Command & Control ----
  {
    ...
    columns: [
      ...
      COL_HP,
      { key: 'coveragePerHullPoint', label: 'Coverage/HP', type: 'number', min: 0, width: 120, description: 'Ship HP covered per system HP (auto-sizes system)' },
      ...
      { key: 'isCore', label: 'Core', type: 'boolean', width: 60, description: 'Computer core — enables control computers when installed. Must also set quality.' },
      { key: 'isRequired', label: 'Required', type: 'boolean', width: 80, description: 'Required system (cockpit/command deck)' },
      { key: 'requiresCore', label: 'Req. Core', type: 'boolean', width: 90, description: 'Requires a computer core to be installed. Appears nested under qualifying cores.' },
      { key: 'isDedicated', label: 'Dedicated', type: 'boolean', width: 90, description: 'Dedicated to a single linked weapon/sensor (fire/sensor controls)' },
      { key: 'linkedSystemType', label: 'Linked', type: 'select', width: 80, description: 'System type this links to (fire/sensor controls)', options: [{ value: '', label: '—' }, { value: 'weapon', label: 'Weapon' }, { value: 'sensor', label: 'Sensor' }] },
      { key: 'quality', label: 'Quality', type: 'select', width: 80, description: 'Quality tier — cores provide this quality; controls are capped by maxQuality', options: [{ value: '', label: '—' }, { value: 'Ordinary', label: 'Ordinary' }, { value: 'Good', label: 'Good' }, { value: 'Amazing', label: 'Amazing' }] },
      { key: 'maxQuality', label: 'Max Quality', type: 'select', width: 100, description: 'Maximum core quality this control can use (e.g. Good control needs Good+ core)', options: [{ value: '', label: '—' }, { value: 'Ordinary', label: 'Ordinary' }, { value: 'Good', label: 'Good' }, { value: 'Amazing', label: 'Amazing' }] },
      { key: 'stepBonus', label: 'Step Bonus', type: 'number', width: 100, description: 'Step bonus/penalty (negative = better, e.g. -1 for Ordinary fire control)' },
      { key: 'maximumLinkedSystemSize', label: 'Max Link HP', type: 'number', min: 0, width: 100, description: 'Max HP of linked weapon/sensor (attack computer limit)' },
      { key: 'maxShipHullPoints', label: 'Max Ship HP', type: 'number', min: 0, width: 110, description: 'Max ship HP this system can be used on (cockpit only)' },
      { key: 'lifeSupportCoverageHp', label: 'LS Coverage', type: 'number', min: 0, width: 110, description: 'Life support coverage in HP (cockpit only)' },
      { key: 'effect', label: 'Effect', type: 'text', width: 250, description: 'Special effect description' },
      COL_DESC,
    ],
    defaultItem: { id: '', name: '', progressLevel: 6, techTracks: [], category: 'command', hullPoints: 1, coveragePerHullPoint: 0, maxHullPoints: 0, powerRequired: 0, cost: 50000, costPer: 'unit', linkedSystemType: '', maxQuantity: 0, isCore: false, isRequired: false, requiresCore: false, isDedicated: false, quality: '', maxQuality: '', stepBonus: 0, maximumLinkedSystemSize: 0, maxShipHullPoints: 0, lifeSupportCoverageHp: 0, effect: '', description: '' },
  },
  ...
  // ---- Hangar & Misc ----
  {
    ...
    columns: [
      ...
      COL_EXPANDABLE, COL_EXP_VALUE_PER_HP, COL_EXP_COST_PER_HP,
      { key: 'hullPercentage', label: 'Hull %', type: 'number', min: 0, max: 100, width: 70, description: 'Percentage of hull used' },
      { key: 'hangarCapacity', label: 'Hangar Cap.', type: 'number', min: 0, width: 100, description: 'Hangar capacity in hull points per hull point allocated' },
      { key: 'dockCapacity', label: 'Dock Cap.', type: 'number', min: 0, width: 90, description: 'Docking capacity in hull points' },
      { key: 'ordnanceCapacity', label: 'Ord. Cap.', type: 'number', min: 0, width: 90, description: 'Ordnance storage capacity per hull point' },
      { key: 'cargoCapacity', label: 'Cargo Cap.', type: 'number', min: 0, width: 90, description: 'Cargo capacity in cubic meters' },
      { key: 'evacCapacity', label: 'Evac Cap.', type: 'number', min: 0, width: 90, description: 'Evacuation capacity in people' },
      { key: 'evacCapacityPerHP', label: 'Evac/HP', type: 'number', min: 0, width: 80, description: 'Evacuation capacity per hull point' },
      { key: 'prisonersCapacity', label: 'Prisoner Cap.', type: 'number', min: 0, width: 110, description: 'Prisoner capacity' },
      { key: 'scientistCapacity', label: 'Scientist Cap.', type: 'number', min: 0, width: 110, description: 'Scientist capacity' },
      { key: 'bedCapacity', label: 'Bed Cap.', type: 'number', min: 0, width: 80, description: 'Medical bed capacity' },
      { key: 'fuelCollectionCapacity', label: 'Fuel Coll.', type: 'number', min: 0, width: 90, description: 'Fuel collection capacity' },
    ...
    defaultItem: { id: '', name: '', progressLevel: 6, techTracks: [], category: 'cargo', hullPoints: 1, powerRequired: 0, powerPer: 'unit', cost: 10000, costPer: 'unit', expandable: false, expansionValuePerHp: 0, expansionCostPerHp: 0, hullPercentage: 0, effect: '', description: '' },
  },
...
export const EDITOR_SECTION_GROUPS: EditorSectionGroup[] = [
  ...
  { id: 'support', label: 'Support Systems', sectionIds: ['lifeSupport', 'accommodations', 'storeSystems', 'gravitySystems', 'artificialGravityRules'] },
