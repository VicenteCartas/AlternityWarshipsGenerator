...
- **Workshop shell polish:** Replaced the promotional card grid with a compact task-oriented tool launcher. Character Creator, Warships Generator, and Battle Resolution share one responsive module start screen with a consistent Workshop Home bar, icon/title/version header, highlighted New action, and grouped Start, Manage, or Planned actions. Travel Calculator, Star System Generator, and Civilization Builder now use the same left-gutter Workshop Home bar above their tool headers instead of separate app-bar arrows. Every top-level tool transition resets the page to the top before paint, so scroll position from Workshop Home or the previous tool is never carried into the next view. Warships retains autosave recovery and Getting Started, while Character keeps visible planned Library and Sources & Mods entries. Native File, View, Help, recent-file, start-screen, and Workshop Home commands follow the active tool; Character and Battle files populate type-filtered recent lists, and shortcut help reflects the active context.
