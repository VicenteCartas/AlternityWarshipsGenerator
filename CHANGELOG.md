...
- **Redesigned character sheet exports:** The NPC/Condensed profile now follows the PHB fast-play template's true half-page, single-column layout: one Letter page holds two half-width columns, with the main profile always starting in column 1 (no page header/branding, just the hero name as the first line) and column 2 reserved for overflow or extended sections (Psionics, Mutations, Cybertech, FX) when the character has them. Durability renders as two side-by-side sub-columns to save vertical space, and trained broad and specialty skills are now nested with two levels of indentation under their ability. The Player Character sheet now mirrors the official back-of-book layout: a Hero Sheet page (identity, abilities, action check, durability, game/personal data, attack forms), a dedicated Skills page listing the complete skill catalogue with computed trained and untrained scores so every roll is ready, a dedicated Extras page (weapon data, equipment, notes), and a Supplemental page for Psionics, Mutations, Cybertech, Computers, and FX that omits any section the character doesn't use. Weapon tables now show the trained skill's display name instead of its internal ID, split the damage type (e.g. HI/O) into its own column from the damage dice, and armor entries are labeled Low Impact/High Impact/Energy everywhere they appear.
...
- **Character Creator rules foundation:** Added versioned PHB and planned accessory source-pack metadata, canonical species and profession definitions, verified character-creation rules tables, and tested pure calculations for ability validation, skill budgets, resistance modifiers, Action Check, Last Resorts, actions per round, movement, durability, skill scores, and Strength damage adjustments.
- **Character Creator PHB skills:** Added all 40 broad and 125 specialty skills from the PHB core skill list, including ability groups, profession discounts, untrained restrictions, broad-skill prerequisites, and named specializations. The catalogue records concise milestone and effect summaries for all 68 specialty skills with PHB rank benefits; affected broad-skill rows show coverage counts, specialty rows show milestone cadences, and searchable dialogs distinguish attained ranks. A tested level-1 purchasing engine now handles free broad-skill surrender, purchased broad-skill limits, cumulative ranks up to rank 3, Diplomat secondary-profession pricing, point budgets, and validation errors.
- **Character Creator PHB psionics:** Added the four psionic broad skills and all 28 specialty skills, with Mindwalker, talent, Diplomat/Mindwalker, and Fraal access rules; path-specific costs and rank limits; favored-discipline modifiers; psionic energy totals, use, fatigue, extension, and recovery; and shared skill-point validation. Source-pack resolution now supports dependencies, additive sections, and section-level replacement so Mindwalking can replace PHB psionics without replacing unrelated PHB rules.
- **Character Creator PHB options:** Added all PHB perks, flaws, Mutant Human advantages and drawbacks, species abilities, profession benefits, and roleplaying fields. Cross-step validation combines their Ability, resistance, durability, Action Check, skill-point, and species-option effects into one level-1 character result.
- **Character Creator PHB equipment:** Added personal equipment, services, computers, 23 cybergear entries, all 100 weapons, and all 31 armor entries. Starting funds, Progress Level, qualities, ammunition, cyber tolerance, training, dependencies, mass, protection, armor penalties, and one shared credit balance are calculated and validated.
- **Character Creator application:** Added the suite-hub entry and an 11-step desktop wizard for identity, species, profession, abilities, skills, perks/flaws, psionics, mutations, cybergear, equipment, and summary. Species and professions use selected-summary panels above searchable, source-filtered, sortable comparison tables designed for additional books and mods. Skills group specialties under collapsible broad-skill rows, with expand/collapse-all controls and automatic reveal of search matches. The stepper uses hidden-scrollbar arrow navigation, compact Warships-style dividers, and status icons for completed, required, and optional steps, including conditional requirements for Mindwalkers and Mutant Humans. Comparison catalogues use searchable paginated tables; complete snapshots support undo/redo and unsaved-change protection.
- **Character files and PDF export:** Characters save through native dialogs as versioned `.character.json` files with source-pack references and defensive migration. Save format 1.1 adds native language and official-sheet identity fields while loading older saves with safe defaults. PDF export now opens a native Save As dialog and offers two purpose-specific formats: a compact NPC Profile following the PHB page 14 character-template order, and a full Player Character Sheet based on the official sheet at the end of the PHB. Both use the same resolved character model, preserve overflow on continuation pages, default beside the saved character when possible, and provide an Open action after export.
- **Complete Character Summary:** Replaced the minimal final step with Overview, Skills, Combat, Gear, and Options & Notes views covering final abilities, resistances, Action Check, durability, movement, every skill and score, attacks, armor, inventory, budgets, species and profession benefits, psionics, mutations, cybertech, and narrative fields.
- **PHB character-sheet completeness:** Every hero now chooses a free native Language at rank 3. Added Gamemaster, Allegiance, Social Status, Contacts, Enemies, and Notes fields from the printed sheet, plus complete presentation of Mechalus intrinsic cybertech: a Good nanocomputer, two neural data slots, and reflex-device circuitry.
- **Character acceptance fixes:** Species-granted specialty ranks, including the Sesheyan's free Zero-G Training rank 1, now appear in Summary and both PDF formats without duplicating purchased ranks. Printable sheets now display an incomplete marker and issue count when exported from an invalid character.
- **PHB character advancement:** Character creation now accepts a target level and adds a sequential Advancement step after Equipment. Each level grants and carries the PHB skill-point allotment, supports new broad and specialty skills one rank at a time, psionic advancement, profession-priced achievement benefits from Table P29, Ability increases, new perks, flaw removal, contacts, Last Resort replenishment, user-entered campaign credits, and purchased or granted equipment. Summary and both PDF formats show final advanced statistics and a per-level history.
- **Advanced character saves:** Character save format 1.2 records the target level and level-by-level advancement ledger. Existing files migrate as level-1 characters with no advancement history.
- **Structured roleplaying attributes:** Motivation, Moral Attitude, and Character Traits now use editable suggestion menus. The PHB permits custom descriptions, so freeform values and existing saves remain supported. Character Trait 1 and Character Trait 2 are separate controls, fixing the disappearing-comma input bug.
- **Complete PHB Table P33 equipment:** Added all 54 missing Clothing and Accessories, Communications, Medical Gear, pharmaceutical, and Professional Equipment entries with their printed PL, mass, cost, formula, and per-dose values. Equipment now has dedicated filters for all four categories, including the fast-play templates' Comm Gear.
- **Equipment Progress Level filter:** General Equipment, Weapons, and Armor now share a multi-select PL filter. It defaults to the character's current PL, an empty selection shows all PLs, and any combination of explicit PLs shows only exact matches.
- **Cybergear Progress Level filter:** Cybergear now uses the same multi-select PL behavior as Equipment, defaulting to the character's current PL while supporting all or multiple exact levels.
- **Character catalogue filter consistency:** Skills, Perks & Flaws, Psionics, Mutations, and Cybergear now include Book/Mod source filters after their search fields. Equipment and Weapons & Armor place Search, Progress Levels, and Source together on one row, with category tabs on a separate row. The free native Language is now edited within the Knowledge specialty list as a fixed, unremovable rank-3 entry instead of occupying a separate block above the catalogue.
- **Official optional skill rules:** Added built-in, independently selectable support for Optional Rules 2A/2B and 2C from the Alternity development team. The paired starting-skill rules use `30 + (3 x INT)` skill points and `6 + INT resistance modifier` broad skills while retaining the Human's +5 skill points and +1 broad skill; Rule 2C applies flat specialty-rank costs during core and psionic creation and advancement. Active rules appear in summaries and PDFs, changing them after purchases requires confirmation, and character save format 1.3 preserves the selection while older files retain standard PHB behavior.
- **Suite tool order:** Character Creator now appears first on the suite hub, followed by Warships Generator.
- **Workshop document workflow:** All eight tools now implement one capability contract and responsive launch pattern with Workshop Home, New, Open, and up to five type-filtered recent files. Star Sector Generator, Star System Generator, Civilization Builder, Alien Artifact Designer, and Travel Calculator open on launch screens instead of creating documents immediately; their editors expose the same Save and Save As semantics as Character, Warships, and Battle. Native File menus distinguish launch/editor modes, provide Start Screen commands, and keep ten recent files independently for each document type. Tool transitions reset the page to the top before paint. Optional Library, Mods, and PDF capabilities remain tool-specific; Warships retains recovery and Getting Started, while Character keeps its planned Library and Sources & Mods entries.
- **Help links:** Removed duplicate GitHub and issue links from the native Help menu, kept them in About, and corrected repository links to the existing `AlternityWarshipsGenerator` project.
- **Stable Ability validation:** The Character Creator's Ability warning now switches to an equally sized success banner when the allocation is valid, preventing the score fields and navigation from shifting during point assignment.
- **Repeated skill specializations:** Subject- and type-based specialties can now be purchased repeatedly with independent names and ranks, such as Language (English) and Language (Spanish). Duplicate subjects are rejected case-insensitively, while fixed-name specialties remain limited to one purchase.
- **Character equipment clarity:** Cybergear now identifies its one-time training charge as normal skill points and shows the hero's total available and final remaining skill points. Equipment plus Weapons & Armor use compact search fields and Book/Mod source filters. Their paginated catalogues no longer have an inner vertical scrollbar, leaving the page as the sole vertical scroller while horizontal overflow remains local to each table.
- **Starting-credit purchase gate:** New equipment, weapon, and armor purchases remain disabled until all starting-funds dice and any wealth check are complete. Pre-roll views now explain the requirement instead of showing a misleading zero or negative balance, while existing selections can still be removed.
- **Star Sector Generator:** Added the GMG Table G57 province, cluster, region, arm, and galaxy scales with selectable Workshop GMG and science-informed generation models. Reproducible seeds create a manageable 3D campaign map while retaining the scale's separate background-star population, with configurable mapped and highlighted systems, spectral classes, roles, hooks, spatial features, connected travel routes, and exact three-dimensional distances. Factions receive editable capitals, influence, claims, and hex borders; systems, factions, and features support locks and targeted regeneration. Highlighted systems can be developed into deterministic full star systems, embedded in the sector, and imported or exported as standalone `.system.json` files. Sectors use native `.sector.json` New/Open/Save/Save As/recent workflows and export a map-and-gazetteer PDF.
- **Star System Generator:** Added selectable GMG Rules and Science-informed generation. GMG mode implements tables G58-G68 for stars, orbit tracks, planetary types, moons and rings, environment classes and GRAPH ratings, oceans, climates, landforms, and life-series rolls. Science-informed mode is reproducible from a visible seed and configurable system density, observed/conservative/optimistic planet occurrence, and life frequency; it generates stellar mass, radius, luminosity, temperature, age, metallicity, stable multiple-star architecture, mutually Hill-spaced planets, Kepler periods, mass, radius, density, gravity, rotation and tidal locking, atmosphere retention, water, habitable-zone position, and simplified habitability and life outcomes. Every science planet also receives a clearly labeled GMG game translation: planet category, nearest G64 Environment Class, G/R/A/P/H code and names, oceans, climate, landforms, and Life Series. Gravity, pressure, and heat follow the GMG's physical bands; radiation, magnetic protection, atmospheric breathability, greenhouse warming, and Environment Class matching are explicitly identified as inferred approximations. Both physical and game results persist in `.system.json` files and appear in model-aware PDFs.
- **GMG Civilization Builder:** Added a separate authored worksheet for the GMG's civilization procedure. It combines Progress Level, Civilization Level, Law Level, resource and law modifiers, live minimum-technology and anarchy warnings, and structured sections for culture, economy, repeatable cities and towns, repeatable stations and installations, alien or mixed societies, and campaign roles. Economy includes the complete 40-entry Possible Trade Commodities table with value, bulk, restricted status, search, and import/export selection. Each location has its own editor with duplicate and remove actions; installations use the GMG's 13-item facility checklist. Civilizations save as versioned `.civilization.json` files, migrate the earlier single-location draft, and export either a complete civilization PDF or individual location sheets. Unsaved-change protection covers destructive navigation in both setting tools.
- **GMG Alien Artifact Designer:** Added the complete Alien Artifacts procedure from GMG pp. 164-175 under Build a Setting. Design manually or generate from Tables G51-G56 using seven forms and their subtypes, eleven purposes, all 44 powers at Ordinary/Good/Amazing quality, all 12 drawbacks at Slight/Moderate/Extreme severity, and the exact eight perk/eight flaw balance packages. Perk and flaw acquisition includes an explicit d8 roll beside the package selector. Power and drawback tabs explain that their roll buttons replace the current lists with random entries matching the selected package’s qualities, purposes, and severities; a required missing secondary purpose is rolled automatically from G52. Form, purpose, powers, and drawbacks can be locked and rerolled independently. Continuous validation checks purpose alignment and package counts; story fields cover creator, origin, appearance, activation, history, ownership, factions, secrets, and campaign hooks. Artifacts save as versioned `.artifact.json` documents with native New/Open/Save/Save As/recent workflows and export complete PDF dossiers.
- **Travel Calculator module:** New sublight trip planner for spacecraft using either Warships acceleration ratings, physical acceleration in g or m/s^2, or an imported `.warship.json` design.
  - Trips save through native dialogs as versioned `.travel.json` documents, preserving every calculator input and imported ship profile. New, Open, Save, Save As, recent trips, Start Screen navigation, and unsaved-change protection match the other Workshop document tools.
  ...
  - Warships ratings and imported designs offer selectable acceleration conversion. Scale-derived conversion remains the default (`Acc 1` is about `0.0567 g` at PL6 and `113.3 g` at PL7+); Warships-published conversion reproduces the sourcebook interpretation (`Acc 1` is about `17 g` at PL6 and `3,399 g` at PL7+). Direct physical inputs in `g` or `m/s^2` are unchanged.
...
- **Battle Resolution module:** New tool implementing the abstract combat system from *The Externals* (pp. 63-73). Build two opposing forces, split them across simultaneous theatres, and resolve each theatre round by round.
  ...
  - **Specific casualty allocation.** Scenarios can retain the sourcebook's abstract proportional losses or track losses against named stacks. Tracked rounds pause for editable suggestions, protect designated priority assets by default, and permit one partial stack to match the target CS. Capital categories and imported heavy or super-heavy warships default to priority status, which remains editable per stack.
  - **Scenario victory objectives.** Add asymmetric, battle-wide or theatre-specific objectives for withdrawal, total destruction, priority assets, selected categories or named units, category strength thresholds, and preservation. Achieved objectives can conclude a theatre or be acknowledged while combat continues.
  - **Battle results and report export.** A fifth Results step summarizes theatre winners, unresolved engagements, withdrawal status, achieved or failed objectives, remaining CS by stack and category, and tracked starting, surviving, damaged, and destroyed quantities. Abstract engagements explicitly leave specific casualties unallocated. The PDF after-action report includes the same results, casualty status, objectives, order of battle, and round log.
...
- **Fixed missing capacity fields in Hangar & Misc mod editor:** The mod editor schema for `hangarMiscSystems` was missing all capacity columns (`cargoCapacity`, `hangarCapacity`, `dockCapacity`, `ordnanceCapacity`, `evacCapacity`, `prisonersCapacity`, `scientistCapacity`, `bedCapacity`, `fuelCollectionCapacity`, `powerPointsCapacity`, `cargoServiceCapacity`, `troopCapacity`, `coveragePerHullPoint`, `evacCapacityPerHP`). Modders could not view or edit these values through the UI. All capacity fields are now exposed as optional numeric columns.
- **Duplicate Mod:** Added a Duplicate button to the Mod Manager that copies a mod and all its data files into a new folder named "… (Copy)", disabled by default.
- **Mod editor: full computer core & control computer support:** Added 9 missing columns to the Command & Control mod editor section (`isCore`, `requiresCore`, `isDedicated`, `isRequired`, `maxQuality`, `stepBonus`, `maximumLinkedSystemSize`, `maxShipHullPoints`, `lifeSupportCoverageHp`). Modders can now create custom computer cores and dedicated control computers that behave identically to baseline systems.
...
- **FTL Drive Splitting**: All FTL drives now offer Split ×2 / Split ×4 buttons (matching defense screen behavior), plus an "Exceeds Zone Limit" warning chip when the drive exceeds the zone limit. Sub-sections appear independently in the Damage Zones step while the drive remains a single system for power/cost/rating purposes.
...
- **Replaced blocking `alert()` with snackbar notifications in image upload (review 4.2):** Image upload validation errors (file too large, wrong type) used `alert()` while the rest of the app uses the snackbar notification system. Replaced with `onShowNotification` and deleted the orphaned `DescriptionTab.tsx` duplicate.
- **Fixed Fortress Ship crew count:** Corrected crew from 120,000 to 12,000 to match the sourcebook.
- **Show warhead area-of-effect info in weapon magazines:** The magazine edit form's available warheads table, launch system ordnance loading table, and hangar magazine ordnance loading table now display an area-effect icon with tooltip (matching the OrdnanceSelection pattern). PDF export magazine rows now include AoE ranges for loaded warheads.
- **Added magazine capacity to Bomb Projector and Bomb Salvo:** Per the rules, both weapons have a 16-point warhead magazine (max warhead size 2), expandable by +4 size points per extra HP at $50K, matching the Accelerator/Heavy Accelerator pattern.
- **Fixed Thermal Inducer/Nullifier max accuracy:** Per Table 5-12 footnote, the maximum accuracy bonus for consecutive fire is -4 steps, not -3.
- **Fixed Null Torpedo missing area-of-effect data:** Per Table 5-11, the Null Torpedo is an area-effect weapon (Amazing 100m, Good 200m, Ordinary 300m, TA). Added the missing `area` property to weapons.json.
...
- **Removed Vite boilerplate button styles from `index.css`:** The global `button` CSS rules (border transition, hover border-color, focus outline, background-color) from the Vite React template were overriding MUI button styling and causing unwanted visual effects. Removed them so MUI buttons render cleanly.
- **Smart auto-assign for damage zones:** The auto-assign algorithm now uses spatial scoring to place systems in contextually appropriate zones instead of simply picking the zone with the most remaining space. Power plants, FTL drives, and command systems prefer deep/center zones; engines go to aft zones; weapons and sensors align with their firing arcs (multi-arc turrets land in the geometric center of their coverage — e.g. a forward+port+starboard turret goes to the forward centerline); sensors without arcs default to fore zones; defenses favor outer zones; hangars stay near the surface. A small balance bonus still spreads systems across zones when preferences are equal.
...
- **Removed deprecated `@types/html2canvas` dev dependency (review 4.3):** `html2canvas@1.4.1` ships its own types; the `@types` package provided stale v0.5 definitions.
- **Eliminated logger no-ops from production bundles (review 4.4):** Wrapped all 29 `logger.*()` call sites in `if (import.meta.env.DEV)` guards so Vite dead-code-eliminates both the calls and their argument expressions in production builds. Logger definition simplified to direct `console.*` bindings.
- **Refactored step completion switch to registry (review 4.5):** Replaced the 13-case `switch` in `stepCompletionService.ts` with a `Record<StepId, CompletionCheck>` registry, providing compile-time exhaustiveness and per-predicate testability.
- **Persisted theme preference via Electron app settings (review 4.6):** Theme mode (`dark`/`light`/`system`) is now stored in a `settings.json` file in the Electron `userData` directory, consistent with how recent files and mod settings are persisted. Falls back to `localStorage` in dev/web mode.
- **Removed unused variable suppressions (review 4.7):** Removed the unused `isRequired` prop from `EngineSelection` (already handled by `StepHeader` in `App.tsx`), removed `eslint-disable` comment from `SensorSelection`, and added `argsIgnorePattern: '^_'` to the ESLint config for the standard unused-callback-parameter convention.
...
- **Moved `WarshipState` to types layer (review 3.1):** Relocated the `WarshipState` interface from `saveService.ts` to `src/types/warshipState.ts`, fixing the inverted dependency where the core runtime model lived in the persistence layer. All consumers now import from the types module. Also fixed pre-existing missing type imports in `DamageDiagramSelection.tsx` and unused imports in `pdfExportService.test.ts`.
- **Replaced module-level ID counters with `crypto.randomUUID()` (review 3.4):** `embarkedCraftService.ts` and `damageDiagramService.ts` used module-level mutable counters for ID generation, causing non-deterministic IDs across test runs and counter leakage between designs. Replaced with `crypto.randomUUID()` to guarantee uniqueness without shared mutable state.
- **Encapsulated dataLoader.ts mutable singletons into factory (review 3.3):** Replaced six module-level mutable variables (`cache`, `pureBaseCache`, `rawBaseData`, `dataLoaded`, `loadPromise`, `activeMods`) with a `createDataStore()` factory that returns a `DataStore` instance. A default instance backs all existing named exports, so no consumer changes were needed. Tests can now create independent stores for isolation.
- **Converted library scanner to async fs operations (review 3.8):** `scanWarshipFilesSync` in `electron/main.ts` used synchronous `readdirSync`/`statSync`/`readFileSync` on the main process, blocking the event loop during library scans. Replaced with async `fs/promises` counterparts so the UI stays responsive.
- **Closed undo history snapshot concern as by design (review 3.5):** Structural sharing already occurs naturally via JavaScript reference semantics — unchanged arrays share the same reference across consecutive snapshots. Per-snapshot overhead is ~500 bytes; 50 snapshots waste ~25 KB total.
- **Eliminated `any` types in DamageDiagramSelection's SystemCollectorConfig (review 3.6):** Made `SystemCollectorConfig` generic over its item type `T` and introduced a `collector<T>()` factory function that infers `T` from the items array, ensuring callbacks match their item type at compile time. Removed all `eslint-disable @typescript-eslint/no-explicit-any` pragmas.
- **Optimized menu updates on mode change (review 4.8):** `set-builder-mode` IPC handler no longer rebuilds the entire Electron menu (which re-read recent files from disk) just to toggle a few `enabled` flags. Added `id` fields to mode-sensitive menu items and a lightweight `updateMenuForMode()` that toggles `enabled` state in-place via `getMenuItemById()`. Full `createMenu()` is still used when menu content actually changes (recent files, app startup).
- **Eliminated `react-hooks/set-state-in-effect` suppressions (review 3.9):** Created a reusable `useAsyncData` hook (`src/hooks/useAsyncData.ts`) for async data loading with cancellation and imperative refresh. Refactored `DesignTypeDialog` and `ModManager` to use the hook instead of manual `useEffect`+`setState`. Replaced `useEffect` state-sync in `OrdnanceDesignDialog` with `key` prop remounting + `useState` initializers. Removed column-change reset effects in `EditableDataGrid` by adding a `key` prop from `ModEditor`. All 7 lint suppressions across 4 components eliminated.
