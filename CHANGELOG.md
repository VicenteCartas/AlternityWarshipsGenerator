...
- **Character files and PDF export:** Characters save through native dialogs as versioned `.character.json` files with source-pack references and defensive migration. Save format 1.1 adds native language and official-sheet identity fields while loading older saves with safe defaults. PDF export now offers an original two-page printable character sheet, automatic continuation and optional-system pages, or the existing detailed report; both formats use the same resolved presentation model.
...
- **Workshop shell polish:** Replaced the promotional card grid with a compact task-oriented tool launcher and added a dedicated Character Creator start screen with New/Open actions plus visible planned Library and Sources & Mods entries. Native File, View, Help, recent-file, start-screen, and Workshop Home commands now follow the active Warships, Character, Battle, or Travel context; Character and Battle files populate type-filtered recent lists, and shortcut help reflects the active tool.
