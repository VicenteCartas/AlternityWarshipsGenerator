...
- **Smart auto-assign for damage zones:** The auto-assign algorithm now uses spatial scoring to place systems in contextually appropriate zones instead of simply picking the zone with the most remaining space. Power plants, FTL drives, and command systems prefer deep/center zones; engines go to aft zones; weapons align with their primary firing arcs (multi-arc turrets land in the geometric center of their coverage — e.g. a forward+port+starboard turret goes to the forward centerline); sensors prefer fore zones; defenses favor outer zones; hangars stay near the surface. A small balance bonus still spreads systems across zones when preferences are equal.
